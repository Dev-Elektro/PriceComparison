...
//srabotka na Enter
document.getElementById("input_text_search")
...
document.querySelector("#choosefile").onclick = function () {  
  ...
  textforsearch=document.querySelector("#input_text_search").value
  eel.start_search_js_for_one(textforsearch)
  }
  else
  {
...
document.querySelector("#openresultfile").onclick = function () {  
   // Call python's random_python function
    ...
    eel.write_toExcel()
    toastr.success("Сохраняем в файл...");
    
    //alert( "Нет файла реестра... ошибка, откройте вручную" );    toastr.error("Нет файла реестра... ошибка, откройте вручную");
}
...
toastr.error("Необходимо внести данные для поиска!");
}
...
eel.expose(my_output_for_one);
function my_output_for_one(out) {
  document.querySelector("#outputpython").innerHTML = document.querySelector("#outputpython").innerHTML + "<br>" + out ;
}
